            ...
            if (ServiceTask.Contains("msdyn_workorder"))
            {
                ...
                if (ServiceTask.Contains("bcs_containerno"))
                {
                    ...
                    else if (((EntityReference)ServiceTask["msdyn_tasktype"]).Id.ToString().ToUpper() == "3716E5B7-56BF-EC11-983E-0022480ABBD6")//dropoff
                    {
                        ...
                        if (ServiceTask.Contains("bcs_location"))
                        {
                            //InventoryTrackingEntity["bcs_currentlocation"] = FullWorkOrder["bcs_location"];
                            InventoryTrackingEntity["bcs_currentlocation"] = ServiceTask["bcs_location"];
                        }
                    }
                    ...
                    string fetchXMLContainer = @"<fetch>" +
                        "<entity name = 'bcs_container'>" +
                        "<attribute name='bcs_name'/>" +
                        "<filter>" +
                        "<condition attribute = 'bcs_containerid' operator= 'eq' value ='" + container.Id + "'/>" +
                        "</filter>" +
                        "</entity>" +
                        "</fetch>";

                    EntityCollection Containers = service.RetrieveMultiple(new FetchExpression(fetchXMLContainer));

                    if (Containers.Entities.Count > 0)
                    {
                        InventoryTrackingEntity["bcs_name"] = Containers.Entities[0]["bcs_name"];
                    }
